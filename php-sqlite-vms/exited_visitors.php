...
if(isset($_COOKIE['floornumberafterlogin'])){
    $floornumber=$_COOKIE['floornumberafterlogin'];
} ?>
<style>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-hover table-striped" id="visitorTBL">
                        ...
                        <tbody>
                            ...
                            $i = 1;
                            if(isset($_COOKIE['floornumberafterlogin'])){
                            $visitors_sql = "SELECT * FROM `visitor_list` where  `floornumber`={$floornumber} AND date(`date_out`) IS NOT NULL AND date(`date_created`) BETWEEN '{$from}' and '{$to}' ORDER BY strftime('%s', `date_created`) desc";
                            }else{
                                 $visitors_sql = "SELECT * FROM `visitor_list` where date(`date_out`) IS NOT NULL AND date(`date_created`) BETWEEN '{$from}' and '{$to}' ORDER BY strftime('%s', `date_created`) desc";  
                            }
                            $visitors_qry = $conn->query($visitors_sql);
                            while($row = $visitors_qry->fetchArray()):
                                $date_created = new DateTime($row['date_created'], new DateTimeZone('UTC'));$date_created->setTimezone(new DateTimeZone('Asia/Kolkata'));
                                
                                if($row['date_out']){
                                    $date_out = new DateTime($row['date_out'], new DateTimeZone('UTC'));
                                    $date_out->setTimezone(new DateTimeZone('Asia/Kolkata'));
                            ...
                            <tr>
                                ...
                                <td class="text-center"><img  width="64px" height="64px" src="upload/<?=$row['userimgurl']?>"></td>
                                ...
                                <td class="">
                                    <div class="lh-1">
                                        ...
                                        <div class="text-secondary">Department/Floor : <?=$row['contact']?></div>
                                        <div class="text-secondary">Meet : <?=$row['remarks']?></div>
                                        <div class="text-secondary">Purpose : <?=$row['reason']?></div>
                                ...
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <a class="btn btn-sm btn-outline-dark rounded-0 view_data" href="./?page=view_visitor&id=<?= $row['visitor_id'] ?>" data-id='<?= $row['visitor_id'] ?>' title="Print Badge"><span class="material-symbols-outlined">subject</span></a>
                                        <?php if(is_null($row['date_out'])): ?>
                                        <a class="btn btn-sm btn-outline-dark rounded-0 exit_data" href="./?page=view_visitor&id=<?= $row['visitor_id'] ?>" data-id='<?= $row['visitor_id'] ?>' title="Check Out"><span class="material-symbols-outlined">logout</span></a>
                                        <?php endif; ?>
