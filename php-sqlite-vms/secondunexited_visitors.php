                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-hover table-striped" id="visitorTBL">
                        ...
                        <tbody>
                            ...
                            $to = $to->format("Y-m-d");
                            $i = 1;
                            $visitors_sql = "SELECT * FROM `visitor_list` where  contact = 'f2' AND date(`date_out`) IS NULL AND date(`date_created`) BETWEEN '{$from}' and '{$to}' ORDER BY strftime('%s', `date_created`) desc ";
...
<script>
    $(function(){
        ...
        $('#filter').click(function(e){
            ...
            var to = $('#date_to').val()
            location.replace(`./?page=secondunexited_visitors&from=${from}&to=${to}`)
        })
