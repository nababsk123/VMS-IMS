...
Class Master extends DBConnection{
        ...
        if(!isset($formToken) || (isset($formToken) && $formToken != $allowedToken)){
            ...
            if(empty($visitor_id)){
                    ...
                    if($_POST['image'] !==""){
                        ...
                        $userimgurl  = $_POST['dbimage'];
                    }
                    
                    $currentstatus ="in";
                    $floornumber  = $_POST['contact'];
                         $sql = "INSERT INTO `visitor_list` (`user_id`, `id_number`, `fullname`, `contact`, `email`, `reason`, `remarks`,`currentstatus`,`floornumber`,`userimgurl`) VALUES ('{$user_id}', '{$id_number}', '{$fullname}', '{$contact}', '{$email}', '{$reason}', '{$remarks}', '{$currentstatus}', '{$floornumber}', '{$userimgurl}')";
                        $qry = $this->query($sql);
                        $resp['status'] = 'success';
    ...
    function exit_visitor(){
        ...
        if(!isset($token) || (isset($token) && $token != $allowedToken)){
            ...
            $update = $this->query("UPDATE `visitor_list` set `currentstatus` = 'out' where `visitor_id` = '{$id}'");
    ...
    function second_today_visitors(){
        $from = date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 00:00:00"));
        $to =  date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 23:59:59"));
        $from = new DateTime($from, new DateTimeZone('Asia/Kolkata'));
        $from->setTimezone(new DateTimeZone('UTC'));
        ...
        $total = $this->querySingle("SELECT COUNT(`visitor_id`) FROM `visitor_list` where date(`date_created`) BETWEEN '{$from}' and '{$to}' AND contact = 'f2' ");
        return number_format($total);
    }

    function third_today_visitors(){
        $from = date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 00:00:00"));
        $to =  date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 23:59:59"));
        $from = new DateTime($from, new DateTimeZone('Asia/Kolkata'));
        $from->setTimezone(new DateTimeZone('UTC'));
        ...
        $total = $this->querySingle("SELECT COUNT(`visitor_id`) FROM `visitor_list` where contact = 'f3' AND date(`date_created`) BETWEEN '{$from}' and '{$to}' ");
        return number_format($total);
    }
    ...
    function second_visitors_not_exited(){
        $from = date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 00:00:00"));
        $to =  date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 23:59:59"));
        $from = new DateTime($from, new DateTimeZone('Asia/Kolkata'));
        $from->setTimezone(new DateTimeZone('UTC'));
        ...
        $total = $this->querySingle("SELECT COUNT(`visitor_id`) FROM `visitor_list` where contact = 'f2' AND date(`date_created`) BETWEEN '{$from}' and '{$to}' and `date_out` IS NULL");
        return number_format($total);
    }

    function third_visitors_not_exited(){
        $from = date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 00:00:00"));
        $to =  date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 23:59:59"));
        $from = new DateTime($from, new DateTimeZone('Asia/Kolkata'));
        $from->setTimezone(new DateTimeZone('UTC'));
        ...
        $total = $this->querySingle("SELECT COUNT(`visitor_id`) FROM `visitor_list` where contact = 'f3' AND date(`date_created`) BETWEEN '{$from}' and '{$to}' and `date_out` IS NULL");
        return number_format($total);
    }
    ...
    function second_visitors_exited(){
        $from = date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 00:00:00"));
        $to =  date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 23:59:59"));
        $from = new DateTime($from, new DateTimeZone('Asia/Kolkata'));
        $from->setTimezone(new DateTimeZone('UTC'));
        ...
        $total = $this->querySingle("SELECT COUNT(`visitor_id`) FROM `visitor_list` where contact = 'f2' AND date(`date_created`) BETWEEN '{$from}' and '{$to}' and `date_out` IS NOT NULL");
        return number_format($total);
    }

    function third_visitors_exited(){
        $from = date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 00:00:00"));
        $to =  date("Y-m-d h:i:s", strtotime(date("Y-m-d"). " 23:59:59"));
        $from = new DateTime($from, new DateTimeZone('Asia/Kolkata'));
        $from->setTimezone(new DateTimeZone('UTC'));
        ...
        $total = $this->querySingle("SELECT COUNT(`visitor_id`) FROM `visitor_list` where contact = 'f3' AND date(`date_created`) BETWEEN '{$from}' and '{$to}' and `date_out` IS NOT NULL");
        return number_format($total);
    }
