...
Class LoginRegistration extends DBConnection{
                ...
                if($password_verify){
                    if($data['status'] == 1){
                        ...
                        if(array_key_exists('floor', $_POST)){
                            setcookie('floornumberafterlogin', $_POST['floor'], time() + (86400 * 30), "/", $_SERVER['SERVER_NAME'] );
                        }
                        foreach($data as $k => $v){
    ...
    function logout(){
        ...
        setcookie('floornumberafterlogin', null, time() - 3600, "/");
        session_destroy();
        header("location:./");
    }
    ...
    function save_user(){
        ...
        if(!isset($formToken) || (isset($formToken) && $formToken != $allowedToken)){
            ...
            if(!empty($user_id)){
                ...
                $sql = "UPDATE `user_list` set `fullname` = '{$fullname}', `username` = '{$username}', `password` = '{$password}', `status` = '{$status}', `type` = '{$type}' , `floornumber` = '{$floornumber}'where `user_id` = '{$user_id}'";
            }else{
                $sql = "INSERT INTO `user_list` (`fullname`, `username`, `password`, `status`, `type`,`floornumber`) VALUES ('{$fullname}', '{$username}', '{$password}', '{$status}', '{$type}','{$floornumber}')";
            }
